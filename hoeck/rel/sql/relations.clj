...
(def new-pkeys (map #(symbol (apply str %&)) (repeat "new") (iterate inc 0)))
...
(deftype indexed-relation [sql-expr imap ikeys] [IPersistentSet IFn ILookup]
  ...
  (.cons [o] ;; only allow assoc of tuples!
         (when-not (map? o) (throwf "Only tuples may be conjed onto relations, not %s" o))
         (let [pk (select-keys o ikeys)
           ...
           (indexed-relation. sql-expr
                              ...
                              (let [m (meta this)] ;; record updated pkeys in R's metadata
                                (assoc :updates (conj (:updates m #{}) pk)))
                              {})))
